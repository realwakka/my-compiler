...
impl Block {
    fn codegen<'ctx>(&self, cg: &mut CodeGen<'ctx>, function: FunctionValue<'ctx>) -> anyhow::Result<BasicBlock<'ctx>> {
...
fn expr_codegen<'ctx>(expr : &Expr, cg: &mut CodeGen<'ctx>, function: FunctionValue<'ctx>) -> anyhow::Result<IntValue<'ctx>> {
    match expr {
	Expr::Num(val) => {
	    Ok(cg.context.i32_type().const_int(*val, false))
	},
	Expr::Add(lhs, rhs) => {
	    let lhs_value = expr_codegen(lhs, cg, function)?;
	    let rhs_value = expr_codegen(rhs, cg, function)?;
	    let value = cg.builder.build_int_add(lhs_value, rhs_value, "sum").unwrap();	    
	    Ok(value)
	},
	Expr::Sub(lhs, rhs) => {
	    let lhs_value = expr_codegen(lhs, cg, function)?;
	    let rhs_value = expr_codegen(rhs, cg, function)?;
	    let value = cg.builder.build_int_sub(lhs_value, rhs_value, "sub").unwrap();	    
	    Ok(value)
	},
	Expr::Mul(lhs, rhs) => {
	    let lhs_value = expr_codegen(lhs, cg, function)?;
	    let rhs_value = expr_codegen(rhs, cg, function)?;
	    let value = cg.builder.build_int_mul(lhs_value, rhs_value, "mul").unwrap();
	    Ok(value)
	},
	Expr::Div(lhs, rhs) => {
	    let lhs_value = expr_codegen(lhs, cg, function)?;
	    let rhs_value = expr_codegen(rhs, cg, function)?;
	    let value = cg.builder.build_int_signed_div(lhs_value, rhs_value, "sub").unwrap();
	    Ok(value)
	},
	Expr::Bigger(lhs, rhs) => {
	    let lhs_value = expr_codegen(lhs, cg, function)?;
	    let rhs_value = expr_codegen(rhs, cg, function)?;
	    let value = cg.builder.build_int_compare(IntPredicate::SGT, lhs_value, rhs_value, "sub").unwrap();
	    Ok(value)
	},
	Expr::Smaller(lhs, rhs) => {
	    let lhs_value = expr_codegen(lhs, cg, function)?;
	    let rhs_value = expr_codegen(rhs, cg, function)?;
	    let value = cg.builder.build_int_compare(IntPredicate::SLT, lhs_value, rhs_value, "sub").unwrap();
	    Ok(value)
	},
	Expr::Var(name) => {
	    Ok(cg.function_args.get(name).unwrap().clone())
	},
	Expr::Call(name, exprs) => {
	    let function = cg.functions.get(name).unwrap().clone();
	    let args : Vec<BasicMetadataValueEnum<'ctx>>
		= exprs.iter().map(|expr| expr_codegen(expr, cg, function).unwrap().into()).collect();
	    Ok(cg.builder.build_call(function, &args[..], "call").unwrap().try_as_basic_value().left().unwrap().into_int_value())
	},
	Expr::If(cond, a, b) => {
